/**
 * The non-blocking (async) example for getting a release.
 *
...
void get_release_async(const String &releaseId);
void get_release_async2(const String &releaseId);
void get_release_await(const String &releaseId);
...
void loop()
{
    ...
    if (app.ready() && !taskComplete)
    {
        ...
        String releaseId = "cloud.firestore/(default)";

        get_release_async(releaseId);

        // get_release_async2(releaseId);

        // get_release_await(releaseId);
    }
...
void get_release_async(const String &releaseId)
{
    Serial.println("Getting a release...");

    // Async call with callback function.
    releases.get(aClient, FIREBASE_PROJECT_ID, releaseId, processData, "releasesTask");
}
...
void get_release_async2(const String &releaseId)
{
    ...
    releases.get(aClient, FIREBASE_PROJECT_ID, releaseId, rulesResult);
}
...
void get_release_await(const String &releaseId)
{
    ...
    String payload = releases.get(aClient, FIREBASE_PROJECT_ID, releaseId);
