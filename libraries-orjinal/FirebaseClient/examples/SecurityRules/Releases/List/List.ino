...
void list_async(ListOptions &listOptions);
void list_async2(ListOptions &listOptions);
void list_await(ListOptions &listOptions);
...
void loop()
{
    ...
    if (app.ready() && !taskComplete)
    {
        ...
        list_async(listOptions);

        // list_async2(listOptions);

        // list_await(listOptions);
    }
...
void list_async(ListOptions &listOptions)
{
    Serial.println("Listing the release values...");

    // Async call with callback function.
    releases.list(aClient, FIREBASE_PROJECT_ID, listOptions, processData, "releasesTask");
}
...
void list_async2(ListOptions &listOptions)
{
    ...
    releases.list(aClient, FIREBASE_PROJECT_ID, listOptions, rulesResult);
}
...
void list_await(ListOptions &listOptions)
{
    ...
    String payload = releases.list(aClient, FIREBASE_PROJECT_ID, listOptions);
