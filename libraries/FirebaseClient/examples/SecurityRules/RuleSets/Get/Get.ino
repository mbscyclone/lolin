...
void get_ruleset_async(const String &ruleSetId);
void get_ruleset_async2(const String &ruleSetId);
void get_ruleset_await(const String &ruleSetId);
...
void loop()
{
    ...
    if (app.ready() && !taskComplete)
    {
        ...
        get_ruleset_async(ruleSetId);

        // get_ruleset_async2(ruleSetId);

        // get_ruleset_await(ruleSetId);
    }
...
void get_ruleset_async(const String &ruleSetId)
{
    Serial.println("Getting the ruleset...");

    // Async call with callback function.
    ruleSets.get(aClient, FIREBASE_PROJECT_ID, ruleSetId, processData, "ruleSetsTask");
}
...
void get_ruleset_async2(const String &ruleSetId)
{
    ...
    ruleSets.get(aClient, FIREBASE_PROJECT_ID, ruleSetId, rulesResult);
}
...
void get_ruleset_await(const String &ruleSetId)
{
    ...
    String payload = ruleSets.get(aClient, FIREBASE_PROJECT_ID, ruleSetId);
